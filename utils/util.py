...
def printInfo(level, sender, text, send_time=-1):
    ...
    print(logStr)
    if level >= 3:
        wsa_server.get_web_instance().add_cmd({"panelMsg": text})  # 向UE5传输聆听中 有时候也传输值
        if not config_util.config["interact"]["playSound"]: # 非展板播放
            content = {'Topic': 'Unreal', 'Data': {'Key': 'log', 'Value': text}}   # 向UE5传输日志内容
            wsa_server.get_instance().add_cmd(content)
    MyThread(target=__write_to_file, args=[logStr]).start()

def diffPinyin(pinyin1, pinyin2):
    return difflib.SequenceMatcher(None, pinyin1, pinyin2).quick_ratio() > 0.6
