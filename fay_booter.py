...
from scheduler.thread_manager import MyThread
from utils.config_util import video_stream
...
class RecorderListener(Recorder):
    ...
    def on_speaking(self, text):
        ...
        if len(text) > 1:
            ...
            # 设置播放器状态
            feiFei.video_stream.wait_playing = False
            feiFei.video_stream.window.update_listening(True)
    ...
    def get_stream(self):
        ...
        # 获取设备信息
        devInfo = self.paudio.get_device_info_by_index(device_id)
        self.channels = int(devInfo['maxInputChannels'])
        print("麦克风通道",self.channels,devInfo)
        if self.channels == 0:
            util.log(1, '请检查设备是否有误，再重新启动!')
            return
        self.stream = self.paudio.open(input_device_index=device_id, rate=self.__RATE, format=self.__FORMAT, channels=self.channels, input=True,frames_per_buffer=512)
        self.__running = True
...
def console_listener():
    ...
    while __running:
        ...
        if args[0] == 'help':
            ...
            # 清空状态
            feiFei.video_stream.wait_playing = True
            feiFei.video_stream.window.update_listening(True)
            recorderListener.textSimulation(''.join(args[1:]))
        else:
...
def speak(q_msg,text):
    global feiFei
    feiFei.speak(q_msg,text)
def start():
    global feiFei
    global recorderListener
    global __running
    global deviceInputListener
    # video_stream = config_util.video_stream
    # video_stream.window.start_signal.emit(False)
    # video_stream.window.update_listening_qidongx(False)
    ...
    # video_stream.window.start_signal.emit(True)
    # video_stream.window.update_listening_qidongx(True)
    # video_stream.window.update_listening_dengdais(False)
    # video_stream.window.update_listening_dengdaix(False)
    # video_stream.window.update_listening_duihuakuang1(False)
    # video_stream.window.update_listening_duihuakuang2(False)
    # video_stream.window.update_listening_budianzan(False)
    # video_stream.window.update_listening_bucai(False)
    # # 暂停按钮显示出来
    # video_stream.window.update_pause(False)
