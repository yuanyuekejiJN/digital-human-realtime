new Vue({
    ...
    data() {
        return {
            ...
            page_size:10,
            is_edit:false,
            current_page_shuziren:1,
            page_size_shuziren:10,
            videoUrl:null,
            imageUrl:null,
            is_edit_shuziren:false
        }
    },
    created() {
        this.postStartLive()
    ...
    methods: {
        handleVideoSuccess(res, file) {
            console.log(res)
        this.shuziren.video_raw = res.raw;
         // this.shuziren.video = res.msg;
         this.$set(this.shuziren, 'video', res.msg);
      },
         handleVideo2Success(res, file) {
             console.log(res)
        this.shuziren.video2_raw = res.raw;
        // this.shuziren.video2 = res.msg;
          this.$set(this.shuziren, 'video2', res.msg);
      ...
      beforeVideoUpload(file) {
        ...
        handleShuzirenSizeChange(e){
            console.log(e)
            this.page_size_shuziren = e
            this.getShuzirenList()

        },
        handleShuzirenCurrentChange(e){
            console.log(e)
            this.current_page_shuziren = e
            this.getShuzirenList()
        },
         // 回车和空格键提交右侧信息
        handkeyCode(e) {
        ...
        connectWS() {
            ...
            socket = new WebSocket('ws://'+currentDomain+':10003')
                ...
                if (!executed && xhr.status === 200) {
                    try {
                        if (xhr.responseText.length > 0) {
                            ...
                            _this.open_chufachi=attribute["open_chufachi"]
                             _this.open_huanxingchi=attribute["open_huanxingchi"]
                            _this.chufachi=attribute["chufachi"]
                            _this.huanxingchi=attribute["huanxingchi"]
                            ...
                            _this.cache_model = attribute["cache_model"]
                            _this.cache_model_content = attribute["cache_model_content"]
                            let item_data_list = []
            ...
            let send_data = {
                "config": {
                    ...
                    "attribute": {
                        ...
                        "open_chufachi":this.open_chufachi,
                        "open_huanxingchi":this.open_huanxingchi,
                        "huanxingchi":this.huanxingchi,
                        "chufachi":this.chufachi,
                        "cache_model":this.cache_model,
                        "cache_model_content":this.cache_model_content
                    },
                    "interact": {
        ...
        send(sendto) {
         ...
         add_question() {
            ...
            if(_this.is_edit){
                _this.edite_q_r(_this.question)
                return;
            }
            if (!_this.question.question) {
                alert('请输入问题');
                return;
            }
            if (!_this.question.answer) {
                alert('请输入答案');
                return;
            }

            let url = "/api/add_question";
            let send_data = _this.question;
        ...
        add_shuziren() {
            let _this = this;
            console.log(_this.shuziren)
            if(_this.is_edit_shuziren){
                _this.edite_q_r_shuziren(_this.shuziren)
                return;
            }
            if (!_this.shuziren.name) {
                alert('请输入名字');
                return;
            }
            if (!_this.shuziren.sound) {
                alert('请输入声音');
                return;
            }
             if (!_this.shuziren.name) {
            ...
            if (!_this.shuziren.sound) {
             ...
             if (!_this.shuziren.video) {
                alert('请上传无动作视频');
                return;
            }
             if (!_this.shuziren.video2) {
                alert('请上传有动作视频');
                return;
            }

            let url = "/api/add_shuziren";
            let send_data = _this.shuziren;
            ...
            xhr.onload = function () {
                if (xhr.status === 200) {
                    _this.dialogFormVisible = false
                    _this.getShuzirenList()
                    _this.$set(_this, 'shuziren', {});
                    _this.$notify({
                            title: '成功',
                            message: "添加成功",
                            type: 'success'
        ...
        edite_q(e){
         ...
         edite_shuziren(e){
            let _this = this;
               ...
               //      return;
               //  }
             _this.$set(_this, 'shuziren', e);
                 _this.dialogFormVisible = true
            _this.is_edit_shuziren= true
        },
        edite_q_r(e){
          console.log(e)
             let _this = this;
        ...
        getQuestionList(){
            ...
            let url = "/api/get-question";
            let xhr = new XMLHttpRequest()
            xhr.open("post", url)
            xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded")
            xhr.send('data=' + encodeURIComponent(JSON.stringify({
                n:_this.page_size*(_this.current_page-1),
                l:_this.page_size
            })))
            xhr.onload = function () {
                if (xhr.status === 200) {
                    let data =  eval('(' + xhr.responseText + ')')
                            // _this.question_list = data['list'];
                     _this.$set(_this, 'question_list', data['list']);
                    _this.question_count = data['count']
                  console.log('请求成功：', xhr.responseText);
                } else {
                  console.error('请求失败：', xhr.status, xhr.responseText);
                }
         ...
         getShuzirenList(){
            ...
            xhr.onload = function () {
                if (xhr.status === 200) {
                    let data =  eval('(' + xhr.responseText + ')')
                            // _this.question_list = data['list'];
                     _this.$set(_this, 'shuziren_list', data['list']);
                    _this.question_count = data['count']
                  console.log('请求成功：', xhr.responseText);
                } else {
                  console.error('请求失败：', xhr.status, xhr.responseText);
